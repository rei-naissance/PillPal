...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    if (cachedResults) {
      ...
      setIsLoading(false)
      preloadTreatmentData(cachedDiseases)
    } else {
      // Only call API if no cached results exist
      analyzeSymptons(symptomsArray, cacheKey)
    }
  }, [router])
  ...
  return (
    <main className="max-w-4xl mx-auto flex flex-col min-h-screen">
      {/* Header */}
      ...
      {/* Error Message */}
      {error && (
        <aside className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-8 flex items-start gap-4" role="alert">
      ...
      <section className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl border border-gray-100/50 p-8 flex flex-col mb-8" style={{ maxHeight: 'calc(100vh - 280px)' }}>
        {diseases.length > 0 ? (
              ...
              {diseases.map((disease, index) => (
                <li key={index} className="list-none">
                  <button
                    ...
                    className="w-full group relative flex items-center justify-between p-4 bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-green-200 transition-all duration-300 text-left overflow-hidden ring-1 ring-black/5 hover:ring-green-500/20"
                  >
                    <div className="absolute inset-0 bg-gradient-to-r from-green-50/0 via-green-50/0 to-green-50/50 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

                    <div className="relative z-10 flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-gray-100/80 flex items-center justify-center text-gray-400 group-hover:bg-green-50 group-hover:text-green-500 transition-colors shadow-inner border border-gray-100">
                    ...
                    <div className="relative z-10 w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 group-hover:bg-green-500 group-hover:text-white transition-all shadow-sm group-hover:shadow-md group-hover:scale-110 border border-gray-100 group-hover:border-green-500">
          ...
          <div className="text-center py-16 flex-shrink-0">
            <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-8 shadow-inner ring-1 ring-gray-100">
